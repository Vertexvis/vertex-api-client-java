...
public class CreateBatchRequest {
  ...
  * @throws IOException if the JSON Object is invalid with respect to CreateBatchRequest
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!CreateBatchRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!CreateBatchRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `CreateBatchRequest` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      JsonArray jsonArrayvertexvisBatchColonOperations = jsonObj.getAsJsonArray("vertexvis/batch:operations");
      // validate the required field `vertexvis/batch:operations` (array)
      for (int i = 0; i < jsonArrayvertexvisBatchColonOperations.size(); i++) {
        BatchOperation.validateJsonObject(jsonArrayvertexvisBatchColonOperations.get(i).getAsJsonObject());
      };
  }
