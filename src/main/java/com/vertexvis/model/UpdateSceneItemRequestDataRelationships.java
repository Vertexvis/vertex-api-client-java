...
public class UpdateSceneItemRequestDataRelationships {
  ...
  * @throws IOException if the JSON Object is invalid with respect to UpdateSceneItemRequestDataRelationships
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!UpdateSceneItemRequestDataRelationships.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpdateSceneItemRequestDataRelationships.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpdateSceneItemRequestDataRelationships` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // validate the optional field `source`
      if (jsonObj.get("source") != null && !jsonObj.get("source").isJsonNull()) {
        UpdateSceneItemRequestDataRelationshipsSource.validateJsonObject(jsonObj.getAsJsonObject("source"));
      }
  }
