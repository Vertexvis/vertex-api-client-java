...
public class BoundingBox {
  ...
  * @throws IOException if the JSON Object is invalid with respect to BoundingBox
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!BoundingBox.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!BoundingBox.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BoundingBox` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      ...
      Vector3.validateJsonObject(jsonObj.getAsJsonObject("min"));
      // validate the required field `max`
      Vector3.validateJsonObject(jsonObj.getAsJsonObject("max"));
  }
