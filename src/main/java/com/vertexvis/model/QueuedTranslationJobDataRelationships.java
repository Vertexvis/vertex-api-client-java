...
public class QueuedTranslationJobDataRelationships {
  ...
  * @throws IOException if the JSON Object is invalid with respect to QueuedTranslationJobDataRelationships
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!QueuedTranslationJobDataRelationships.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!QueuedTranslationJobDataRelationships.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `QueuedTranslationJobDataRelationships` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
      // validate the optional field `geometrySet`
      if (jsonObj.get("geometrySet") != null && !jsonObj.get("geometrySet").isJsonNull()) {
        GeometrySetRelationship.validateJsonObject(jsonObj.getAsJsonObject("geometrySet"));
      }
      // validate the optional field `partRevision`
      if (jsonObj.get("partRevision") != null && !jsonObj.get("partRevision").isJsonNull()) {
        PartRevisionRelationship.validateJsonObject(jsonObj.getAsJsonObject("partRevision"));
      }
  }
